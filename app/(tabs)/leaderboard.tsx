  ...
  return (
    <Screen padded={false} gradient>
      <View style={{ paddingHorizontal: theme.spacing.lg, paddingTop: theme.spacing.lg }}>
        <Text variant="h1">This week</Text>
        <Text variant="caption" muted>
          Ranked by workouts completed — consistency wins
        </Text>
      </View>
      <FlatList
        data={ranked}
        ...
        ListEmptyComponent={
          loading ? null : (
            <EmptyState
              ...
              subtitle="As your clients complete workouts, they'll climb the board here."
            />
          )
        }
        renderItem={({ item }) => {
          ...
          return (
            <Card elevated={item.rank <= 3}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: theme.spacing.md }}>
                ...
                <View style={{ flex: 1 }}>
                  <Text variant="title">{item.full_name ?? 'Client'}</Text>
                  <Text variant="caption" muted>
                    {item.sets_done} sets logged
                  </Text>
                </View>
